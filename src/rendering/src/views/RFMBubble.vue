<template>
    <EchartBase :option="option" width="1000px" height="400px" />
</template>
...
onMounted(async () => {
    ...
    option.value = {
        ...
        grid: { left: 100, right: 40, top: 80, bottom: 60 }, // 增大左侧间距
        xAxis: { name: '最近一次消费', min: 10 }, // x轴起点从10
        yAxis: { name: '消费频率' },
        series: [{
            type: 'scatter',
            symbolSize: (val: any) => Math.log(val[2] + 1) * 1.5, // 更小
