...
# F: 订单数
# M: 消费总额
now = pd.to_datetime(orders["order_time"]).max() + pd.Timedelta(days=1)
rfm = (
    orders.groupby("user_id")
    .agg(
        {
            "order_time": lambda x: (now - pd.to_datetime(x).max()).days,
            "order_id": "count",
            "amount": "sum",
        }
    )
    .reset_index()
    .rename(
        columns={"order_time": "recency", "order_id": "frequency", "amount": "monetary"}
    )
)
