<template>
  <div style="padding: 24px; position: relative; min-height: 100vh;">
    <h1 style="text-align:center; margin-bottom: 16px; margin-top: 0;">电商项目可视化</h1>
    <!-- 显示数据分析开关，放在大标题下方、标签卡片上方，居右 -->
    <div style="width: 100%; display: flex; justify-content: flex-end; align-items: center; margin-bottom: 12px;">
    ...
    <div v-if="showAnalysis && analysisText[tabs[activeTab].key]"
      style="margin: 10px auto 0; max-width: 900px; background: #eaf6ff; color: #222; border-radius: 12px; border: 2px solid #222; padding: 20px 30px; font-size: 18px; font-family: 'Microsoft YaHei', sans-serif; box-shadow: 0 2px 12px #0002; text-align: left;">
      <div v-html="analysisText[tabs[activeTab].key]" />
    </div>
    <!-- 右下角导出按钮，紧凑靠边 -->
    <div style="position: fixed; bottom: 18px; right: 18px; z-index: 10;">
      <button @click="exportAnalysisImage"
        style="padding: 7px 16px; border: 2px solid #409EFF; border-radius: 8px; background: #fff; color: #409EFF; font-weight: bold; cursor: pointer; font-size: 15px;">导出数据分析结果</button>
    </div>
  </div>
</template>
...
function exportAnalysisImage() {
  // 下载 public/analysis_results/ 下所有图片为zip
  const imgCount = 14; // 1.png ~ 14.png
  const folder = '/analysis_results/';
  const zip = new JSZip();
  let finished = 0;
