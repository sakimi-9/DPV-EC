<template>
  <div style="padding: 24px; position: relative; min-height: 100vh;">
    ...
    <!-- 右上角显示数据分析开关 -->
    <div style="position: absolute; top: 32px; right: 48px; z-index: 10; display: flex; align-items: center; gap: 8px;">
      <span>显示数据分析</span>
    ...
    <!-- 右下角导出按钮 -->
    <div style="position: fixed; bottom: 48px; right: 48px; z-index: 10;">
      <button @click="exportAnalysisImage"
        style="padding: 12px 24px; border: 2px solid #409EFF; border-radius: 8px; background: #fff; color: #409EFF; font-weight: bold; cursor: pointer;">导出数据分析结果</button>
    </div>
    ...
    <div v-if="showAnalysis && analysisText[tabs[activeTab].key]"
      style="margin: 32px auto 0; max-width: 900px; background: #222; color: #fff; border-radius: 12px; padding: 24px 32px; font-size: 18px; font-family: 'Microsoft YaHei', sans-serif; box-shadow: 0 2px 12px #0002;">
